...
def _render_foundation_docs_section(planner) -> list[str]:
    ...
    return [
        ...
        "of what they want true after this cycle. It is not an implementation "
        "spec. Translate it into issues by consulting the discovery findings "
        "and research notes below for the *how* — file locations, options, "
        "best practices, things that need more detail. Issues do not need 1:1 "
        "mapping to bullets; they need to make the work visible to the "
        "implementer.\n\n"
        "**Findings is reference for *how*, not a verdict on *whether*.** "
        "`findings.md` and `.aidlc/research/*.md` describe what exists in the "
        "repo and answer specific investigation questions. They are not a "
        "verdict that 'BRAINDUMP intent is already satisfied' just because "
        "the named systems / files exist. When BRAINDUMP describes a perceived "
        "problem ('feels like a small box', 'overlays compete', 'random "
        "interactions') and findings says the structure is technically OK "
        "(correct dimensions, signals wired, files present), file the rebuild "
        "/ redesign issue anyway. Findings tells you *where* to make the change "
        "and *what options* exist; not *whether* the change is needed.\n\n"
        "**Read the BRAINDUMP's tone to pick the mode.** If the language is "
        "vision / big-build / redesign — 'rebuild', 'big bang', 'recompose', "
        "'stop patching symptoms', enumerated phases / steps / tests, broad "
        "qualitative complaints — scaffold the full work, expect to file many "
        "issues including ones for systems that 'exist', and plan to refine "
        "issues across cycles as you learn more. If the language is scoped-fix "
        "— a short list of named bugs, no redesign rhetoric — keep it tight.\n\n"
        "**Issues are living plans.** As planning cycles deepen exploration, "
        "use `update_issue` to add file references, options, expanded "
        "acceptance criteria, and dependencies. The bar for a good issue is "
        "awareness and visibility into everything the implementer will need — "
        "not minimum word count. Don't lock issue text on cycle 1.\n\n"
